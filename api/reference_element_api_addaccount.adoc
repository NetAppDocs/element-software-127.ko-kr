...
[role="lead"]
를 사용하여 시스템에 새 계정을 추가할 수 `AddAccount` 있습니다. 이 방법을 사용하여 계정이 생성될 때 새 계정 아래에 새 볼륨을 생성할 수도 있습니다. 계정에 대해 지정한 CHAP(Challenge-Handshake Authentication Protocol) 설정은 계정이 소유한 모든 볼륨에 적용됩니다.
