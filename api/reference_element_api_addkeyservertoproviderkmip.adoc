...
[role="lead"]
메소드를 사용하여 지정된 키 공급자에 KMIP(Key Management Interoperability Protocol) 키 서버를 할당할 수 `AddKeyServerToProviderKmip` 있습니다. 할당 중에 서버에 연락하여 기능을 확인합니다. 지정된 키 서버가 이미 지정된 키 공급자에 할당된 경우 아무런 작업도 수행되지 않으며 오류가 반환되지 않습니다. 메소드를 사용하여 할당을 제거할 수 `RemoveKeyServerFromProviderKmip` 있습니다.
