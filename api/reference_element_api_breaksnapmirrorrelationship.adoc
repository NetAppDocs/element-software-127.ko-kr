...
[role="lead"]
요소 웹 UI는 이 메서드를 사용하여 `BreakSnapMirrorRelationship` SnapMirror 관계를 끊습니다. SnapMirror 관계가 손상되면 타겟 볼륨이 읽기/쓰기로 전환되어 소스에서 이탈할 수 있습니다. API 메서드와의 관계를 다시 설정할 수 `ResyncSnapMirrorRelationship` 있습니다. 이 방법을 사용하려면 ONTAP 클러스터를 사용할 수 있어야 합니다.
