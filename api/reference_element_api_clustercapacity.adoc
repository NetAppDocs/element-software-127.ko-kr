...
maxOverProvisionableSpace 를 선택합니다
 a| 
프로비저닝 가능한 최대 공간입니다. 계산된 값입니다. 현재 프로비저닝된 공간과 새 볼륨 크기가 이 수를 초과할 경우 새 볼륨을 생성할 수 없습니다. 값은 다음과 같이 계산됩니다. `maxOverProvisionableSpace = maxProvisionedSpace * maxMetadataOverProvisionFactor`
 a| 
정수
