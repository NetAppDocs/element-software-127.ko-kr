...
[role="lead"]
이 방법을 사용하여 클러스터의 모든 Fibre Channel 노드, iSCSI 노드 및 드라이브에 대한 하드웨어 상태 및 정보를 검색할 수 `GetClusterHardwareInfo` 있습니다. 여기에는 일반적으로 제조업체, 공급업체, 버전 및 기타 관련 하드웨어 식별 정보가 포함됩니다.
