...
[role="lead"]
이 방법을 사용하여 단일 드라이브에 대한 높은 수준의 작동 측정값을 얻을 수 `GetDriveStats` 있습니다. 값은 클러스터에 드라이브를 추가한 후 누적됩니다. 일부 값은 블록 드라이브에 따라 다릅니다. 이 메서드를 실행하면 블록 또는 메타데이터 드라이브 형식에 대한 통계 데이터가 반환됩니다.
