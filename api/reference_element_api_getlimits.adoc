...
[role="lead"]
메소드를 사용하여 API에서 설정한 한계값을 가져올 수 `GetLimits` 있습니다. 이러한 값은 Element 릴리스 간에 변경될 수 있지만 시스템에 대한 업데이트 없이는 변경되지 않습니다. API에서 설정한 제한 값을 알면 사용자 대상 도구에 대한 API 스크립트를 작성할 때 유용할 수 있습니다.


NOTE:  `GetLimits`메소드는 메소드를 전달하는 데 사용된 API 엔드포인트 버전에 관계없이 현재 소프트웨어 버전에 대한 제한을 반환합니다.
