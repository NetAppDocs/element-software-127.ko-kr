...
[role="lead"]
을 사용하여 시스템에서 현재 실행 중인 모든 비동기 메서드와 완료된 비동기 메서드의 결과를 나열할 수 `ListAsyncResults` 있습니다. 를 사용하여 비동기 결과를 쿼리해도 `ListAsyncResults` 완료된 asyncHandles 는 만료되지 않습니다. 을 사용하여 에서 반환된 asyncHandles 를 쿼리할 `ListAsyncResults` 수 `GetAsyncResult` 있습니다.
