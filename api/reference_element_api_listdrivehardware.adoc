...
[role="lead"]
이 방법을 사용하여 노드에 연결된 모든 드라이브를 나열할 수 `ListDriveHardware` 있습니다. 개별 노드에서 이 메서드를 사용하면 드라이브 하드웨어 정보가 반환됩니다. 클러스터 마스터 노드 MVIP에서 이 방법을 사용하면 모든 노드의 모든 드라이브에 대한 정보가 반환됩니다.
