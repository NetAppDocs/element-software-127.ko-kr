...
[role="lead"]
이 방법을 사용하여 생성된 모든 KMIP(Key Management Interoperability Protocol) 키 서버를 나열할 수 `ListKeyServersKmip` 있습니다. 추가 매개 변수를 지정하여 결과를 필터링할 수 있습니다.
...
KMIP 키 서버가 생성된 전체 목록입니다.
 a| 
link:reference_element_api_keyserverkmip.html["KeyServerKmip"] 선정되었습니다
