...
[role="lead"]
메소드를 사용하여 클러스터의 모든 프로토콜 엔드포인트에 대한 정보를 검색할 수 `ListProtocolEndpoints` 있습니다. 프로토콜 엔드포인트는 연결된 가상 볼륨 스토리지 컨테이너에 대한 액세스를 제어합니다.
...
시스템의 각 프로토콜 끝점에 대한 정보가 포함된 개체 목록입니다.
 a| 
xref:reference_element_api_protocolendpoint.adoc[ProtocolEndpoint] 선정되었습니다
