...
[role="lead"]
Element 소프트웨어 웹 UI `ListSnapMirrorAggregates`에서는 이 방법을 사용하여 원격 ONTAP 시스템에서 사용 가능한 모든 SnapMirror 애그리게이트를 나열합니다. Aggregate는 물리적 스토리지 리소스 세트를 설명합니다.
...
ONTAP 스토리지 시스템에서 사용 가능한 애그리게이트 목록입니다.
 a| 
xref:reference_element_api_snapmirroraggregate.adoc[snapMirrorAggregate] 선정되었습니다
