...
[role="lead"]
이 방법을 사용하여 현재 시스템에 있는 가상 볼륨을 나열할 수 `ListVirtualVolumes` 있습니다. 이 방법을 사용하여 모든 가상 볼륨을 나열하거나 하위 집합만 나열할 수 있습니다.
...
현재 시스템에 있는 가상 볼륨을 설명하는 객체 목록입니다.
 a| 
xref:reference_element_api_virtualvolume.adoc[가상볼륨] 선정되었습니다
