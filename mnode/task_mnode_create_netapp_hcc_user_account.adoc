...
* 관리 노드에 새 NetApp HCC 역할을 link:task_mnode_create_netapp_hcc_role_vcenter.html#add-an-asset-to-the-management-node["자산을 추가합니다"]사용합니다.
...
[NOTE]
====
*** vCenter Server 5.0용 NetApp Element 플러그인부터 을 사용하려면 https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html["vCenter 연결 모드"^] NetApp SolidFire 스토리지 클러스터를 관리하는 각 vCenter Server에 대한 별도의 관리 노드에서 Element 플러그인을 등록합니다(권장).
*** vCenter Server 4.10 이하 버전용 NetApp Element 플러그인을 사용하여 를 사용하는 다른 vCenter Server의 클러스터 리소스를 관리하는 https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html["vCenter 연결 모드"^] 것은 로컬 스토리지 클러스터로만 제한됩니다.
...
설치 후 새 자산을 수동으로 추가해야 하는 경우 에서 만든 새 HCC 사용자 계정을 <<vCenter에서 새 사용자 계정을 설정합니다>>사용합니다. 자세한 내용은 을 link:task_mnode_add_assets.html["관리 노드에 컨트롤러 자산을 추가합니다"]참조하십시오.
