...
== 단계 1: ISO 또는 OVA를 다운로드하고 VM을 배포합니다

NetApp 지원 사이트에서 적절한 ISO 또는 OVA를 다운로드하고 VM을 설치합니다.
...
VM 설치가 완료된 후 관리 노드 관리자 사용자를 생성하고 관리 노드 네트워크를 구성합니다.
...
관리 노드를 설정하기 전에 관리 노드와 스토리지 클러스터 간의 시간을 동기화합니다.
...
.. 기본 시간 서버를 추가하려면 각 서버마다 새 줄을 추가하십시오. 기본 시간 서버는 <<set-up-the-management-node,나중에>>에서 사용할 스토리지 클러스터에서 사용하는 NTP 서버와 동일해야 합니다.
+
[listing]
----
vi /etc/ntp.conf
...
명령을 사용하여 관리 노드를 `setup-mnode` 구성합니다.
...
*** *--storage_mvip (-sm) [MVIP 주소]*: Element 소프트웨어가 실행되는 스토리지 클러스터의 관리 가상 IP 주소(MVIP)입니다. link:task_mnode_recover.html#configure-time-sync["NTP 서버 구성"] 중에 사용한 것과 동일한 스토리지 클러스터로 관리 노드를 구성하십시오.
...
설치 ID를 찾고 vCenter 컨트롤러 자산을 추가합니다.
